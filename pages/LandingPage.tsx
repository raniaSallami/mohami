...
const InteractiveDemo = ({ isDark }: { isDark: boolean }) => {
  ...
  return (
    <div className={`relative w-full max-w-4xl mx-auto h-[400px] ${isDark ? 'bg-slate-900' : 'bg-white'} rounded-2xl shadow-2xl ${isDark ? 'border-slate-700' : 'border-slate-200'} border overflow-hidden flex flex-col md:flex-row`} dir="ltr">
      <div className={`w-full md:w-20 ${isDark ? 'bg-slate-800' : 'bg-slate-50'} ${isDark ? 'border-slate-700' : 'border-slate-200'} border-e md:border-e-0 md:border-s flex md:flex-col items-center py-4 gap-4 md:gap-6 z-20 justify-center md:justify-start`}>
      ...
      <div className={`flex-1 relative ${isDark ? 'bg-slate-900' : 'bg-slate-50'} p-6 flex items-center justify-center overflow-hidden`}>
        <div className="relative w-64 h-80 bg-white rounded shadow-lg">
          <div className="p-4 space-y-2">
            ...
            {step === 3 && (
              <>
                <div className="absolute top-20 end-2 w-5/6 h-2 bg-red-200 opacity-50 animate-pulse"></div>
                <div className="absolute top-32 end-2 w-2/3 h-2 bg-red-200 opacity-50 animate-pulse"></div>
              </>
            )}
          </div>
          {step === 1 && (
            <div className="absolute top-0 start-0 w-full h-1 bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.8)] animate-scan"></div>
          )}
        </div>
      </div>

      <div className={`w-full md:w-80 ${isDark ? 'bg-slate-800/50' : 'bg-white/50'} backdrop-blur-md ${isDark ? 'border-slate-700' : 'border-slate-200'} border-s md:border-s-0 md:border-e p-6 flex flex-col`}>
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-slate-900' : 'bg-slate-50'} font-sans ${isDark ? 'text-white' : 'text-slate-900'} overflow-x-hidden scroll-smooth transition-colors duration-300`}>
      ...
      <nav className={`fixed w-full z-50 ${isDark ? 'bg-slate-900/80' : 'bg-white/80'} backdrop-blur-md ${isDark ? 'border-slate-800' : 'border-slate-200'} border-b transition-all duration-300`}>
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => { onNavigate('landing'); window.scrollTo(0, 0); }}>
            <img src="/assets/logo.png" alt={window.__t("المحامي")} className="w-10 h-10 rounded-lg shadow-lg transition object-contain" />
            <span className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} tracking-tight`}>{window.__t("المحامي")}</span>
          </div>
          <div className={`hidden md:flex items-center gap-8 ${isDark ? 'text-slate-300' : 'text-slate-600'} font-medium text-sm`}>
            <button onClick={() => scrollToSection('features')} className="hover:text-yellow-400 transition">{window.__t("المميزات")}</button>
            <button onClick={() => scrollToSection('demo')} className="hover:text-yellow-400 transition">{window.__t("كيف يعمل")}</button>
            <button onClick={() => scrollToSection('pricing')} className="hover:text-yellow-400 transition">{window.__t("الأسعار")}</button>
          </div>
          <div className="flex items-center gap-4">
            <button onClick={() => { window.__i18n?.changeLanguage(window.__i18n.language === 'ar' ? 'fr' : 'ar'); window.location.reload(); }} className={`px-3 py-1 rounded font-medium ${isDark ? 'text-white bg-slate-800' : 'text-slate-700 bg-slate-100'}`}>
            ...
            <button onClick={() => onNavigate('login')} className={`${isDark ? 'text-white' : 'text-slate-900'} font-medium hover:text-yellow-400 px-4 py-2 transition`}>{window.__t("دخول")}</button>
            {allowRegistrations && (
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-12">
            <div className="lg:w-1/2 text-center lg:text-start">
              <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-white/80 border-slate-200'} border text-yellow-400 text-sm font-semibold mb-8`}>
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-yellow-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-yellow-500"></span>
                </span>
                <span>{window.__t("الذكاء الاصطناعي الأول للمحامي التونسي")}</span>
              </div>
              <h1 className={`text-5xl lg:text-7xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} leading-tight mb-8`}>
                ...
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600">{window.__t("بسرعة البرق")}</span>
              </h1>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
                {allowRegistrations && (
                  <button onClick={() => onNavigate('register')} className="w-full sm:w-auto px-8 py-4 bg-yellow-500 text-slate-900 rounded-xl font-bold text-lg hover:bg-yellow-400 transition shadow-lg flex items-center justify-center">
                    ...
                    <svg className="w-5 h-5 me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
                  </button>
                )}
                <button onClick={() => setIsVideoModalOpen(true)} className={`w-full sm:w-auto px-8 py-4 ${isDark ? 'bg-white/5 text-white border-white/10 hover:bg-white/10' : 'bg-slate-900/5 text-slate-900 border-slate-300 hover:bg-slate-900/10'} border rounded-xl font-bold text-lg transition flex items-center justify-center`}>
                  {window.__t("شاهد الفيديو التعريفي")}
                </button>
              </div>
            </div>
      ...
      <section id="features" className={`py-24 ${isDark ? 'bg-slate-950' : 'bg-white'}`}>
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className={`text-3xl md:text-4xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-4`}>{window.__t("أدوات خارقة للمحامي العصري")}</h2>
            <p className={`${isDark ? 'text-slate-400' : 'text-slate-600'} max-w-2xl mx-auto`}>{window.__t("تكنولوجيا مصممة خصيصاً لتناسب احتياجات المحكمة التونسية والإجراءات القانونية المعقدة.")}</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: '🧠', title: window.__t("ذكاء اصطناعي قانوني"), desc: window.__t("نموذج مدرب على مجلة الإجراءات الجزائية ومجلة الالتزامات والعقود لاستخراج الدفوع بدقة.") },
              { icon: '👁️', title: window.__t("OCR متطور للمحاضر"), desc: window.__t("تحويل صور محاضر البحث والوثائق المكتوبة بخط اليد أو الآلة الكاتبة القديمة إلى نص رقمي قابل للبحث.") },
              { icon: '🔒', title: window.__t("سرية تامة للموكلين"), desc: window.__t("بيانات قضاياك مشفرة ولا يتم مشاركتها أو استخدامها لتدريب النماذج العامة.") },
              { icon: '📅', title: window.__t("إدارة مكتب شاملة"), desc: window.__t("لوحة تحكم تفاعلية لمتابعة الجلسات، التنبيهات، والوضع المالي للمكتب.") },
              { icon: '⚖️', title: window.__t("تحديثات قانونية"), desc: window.__t("مواكبة آلية لأحدث التنقيحات القانونية وفقه القضاء التونسي.") },
              { icon: '📱', title: window.__t("مكتبك في جيبك"), desc: window.__t("تطبيق ويب سريع يعمل على الهاتف والحاسوب للوصول للملفات من داخل قاعة المحكمة.") },
            ].map((feature, i) => (
      ...
      <section id="pricing" className={`py-24 ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className={`text-3xl md:text-4xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-4`}>{window.__t("باقات تناسب جميع المحامين")}</h2>
            <p className={`${isDark ? 'text-slate-400' : 'text-slate-600'} max-w-2xl mx-auto`}>{window.__t("اختر الخطة المناسبة لحجم مكتبك وطبيعة عملك. الأسعار بالدينار التونسي.")}</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            <div className={`${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} rounded-2xl p-8 border flex flex-col transition duration-300`}>
              <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-2`}>{window.__t("البداية")}</h3>
              <p className={`${isDark ? 'text-slate-400' : 'text-slate-600'} text-sm mb-6`}>{window.__t("للمحامين المتمرنين والطلبة")}</p>
              <div className={`text-4xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-6`}>0<span className={`text-lg ${isDark ? 'text-slate-500' : 'text-slate-400'} font-normal`}> {window.__t("د.ت / شهر")}</span></div>
              <ul className="space-y-4 mb-8 flex-1">
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-green-500 ms-2">✓</span> {window.__t("إدارة")} {planLimits.basic?.cases ?? 5} {window.__t("قضايا نشطة")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-green-500 ms-2">✓</span> {window.__t("تحليل آلي محدود (10 وثائق/شهر)")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-green-500 ms-2">✓</span> {window.__t("الوصول للمكتبة القانونية")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-green-500 ms-2">✓</span> {window.__t("دعم عبر البريد")}</li>
              </ul>
              <button onClick={() => allowRegistrations ? onNavigate('register') : scrollToSection('contact')} className={`w-full py-3 ${isDark ? 'bg-slate-700 hover:bg-slate-600' : 'bg-slate-200 hover:bg-slate-300'} ${isDark ? 'text-white' : 'text-slate-900'} rounded-xl font-medium transition`}>{allowRegistrations ? window.__t("مجاناً") : window.__t("تواصل معنا")}</button>
            </div>
            <div className={`${isDark ? 'bg-gradient-to-b from-slate-800 to-slate-900 border-yellow-500/50' : 'bg-gradient-to-b from-white to-slate-50 border-yellow-500/50'} rounded-2xl p-8 border flex flex-col relative transform md:-translate-y-4 shadow-2xl`}>
              <div className="absolute top-0 end-1/2 transform translate-x-1/2 -translate-y-1/2 bg-yellow-500 text-slate-900 px-4 py-1 rounded-full text-xs font-bold">{window.__t("الأكثر طلباً")}</div>
              <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-2`}>{window.__t("المحترف")}</h3>
              <p className={`${isDark ? 'text-slate-400' : 'text-slate-600'} text-sm mb-6`}>{window.__t("للمحامين المستقلين")}</p>
              <div className={`text-4xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-6`}>{pricing.pro}<span className={`text-lg ${isDark ? 'text-slate-500' : 'text-slate-400'} font-normal`}> {window.__t("د.ت / شهر")}</span></div>
              <ul className="space-y-4 mb-8 flex-1">
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-yellow-500 ms-2">✓</span> {window.__t("إدارة")} {planLimits.pro?.cases ?? 50} {window.__t("قضية نشطة")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-yellow-500 ms-2">✓</span> {window.__t("تحليل ذكي غير محدود بنموذج AI متطور")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-yellow-500 ms-2">✓</span> {window.__t("OCR غير محدود للمحاضر")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-yellow-500 ms-2">✓</span> {window.__t("تقارير PDF احترافية")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-yellow-500 ms-2">✓</span> {window.__t("تنبيهات الجلسات SMS")}</li>
              </ul>
            ...
            <div className={`${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} rounded-2xl p-8 border flex flex-col transition duration-300`}>
              <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-2`}>{window.__t("المكتب")}</h3>
              <p className={`${isDark ? 'text-slate-400' : 'text-slate-600'} text-sm mb-6`}>{window.__t("لمكاتب المحاماة والشركات")}</p>
              <div className={`text-4xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-6`}>{pricing.enterprise}<span className={`text-lg ${isDark ? 'text-slate-500' : 'text-slate-400'} font-normal`}> {window.__t("د.ت / شهر")}</span></div>
              <ul className="space-y-4 mb-8 flex-1">
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-green-500 ms-2">✓</span> {window.__t("قضايا غير محدودة")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-green-500 ms-2">✓</span> {window.__t("3 حسابات محامين")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-green-500 ms-2">✓</span> {window.__t("لوحة تحكم إدارية متقدمة")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-green-500 ms-2">✓</span> {window.__t("أرشيف سحابي مشفر (100GB)")}</li>
                <li className={`flex items-center ${isDark ? 'text-slate-300' : 'text-slate-700'} text-sm`}><span className="text-green-500 ms-2">✓</span> {window.__t("أولوية الدعم الفني 24/7")}</li>
              </ul>
              <button onClick={() => scrollToSection('contact')} className={`w-full py-3 ${isDark ? 'bg-slate-700 hover:bg-slate-600' : 'bg-slate-200 hover:bg-slate-300'} ${isDark ? 'text-white' : 'text-slate-900'} rounded-xl font-medium transition`}>{window.__t("تواصل معنا")}</button>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
          <div className={`max-w-4xl mx-auto ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'} rounded-2xl p-8 md:p-12 border`}>
            <div className="text-center mb-10">
              <h2 className={`text-3xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-4`}>{window.__t("هل لديك استفسار؟")}</h2>
              <p className={isDark ? 'text-slate-400' : 'text-slate-600'}>{window.__t("فريقنا جاهز للإجابة على جميع أسئلتك حول المنصة.")}</p>
            </div>
            <form className="space-y-6" onSubmit={handleContactSubmit}>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className={`block text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-600'} mb-2`}>{window.__t("الاسم أو اسم المكتب")}</label>
                  <input type="text" value={contactName} onChange={(e) => setContactName(e.target.value)} className={`w-full ${isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300 text-slate-900'} border rounded-lg px-4 py-3 focus:border-yellow-500 outline-none transition`} placeholder={window.__t("الاسم الكامل أو اسم المكتب")} required disabled={isSubmitting} />
                </div>
                <div>
                  <label className={`block text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-600'} mb-2`}>{window.__t("البريد الإلكتروني")}</label>
              ...
              <div>
                <label className={`block text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-600'} mb-2`}>{window.__t("الرسالة")}</label>
                <textarea rows={4} value={contactMessage} onChange={(e) => setContactMessage(e.target.value)} className={`w-full ${isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300 text-slate-900'} border rounded-lg px-4 py-3 focus:border-yellow-500 outline-none transition`} placeholder={window.__t("كيف يمكننا مساعدتك؟")} required disabled={isSubmitting}></textarea>
              </div>
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            <div className="col-span-1 md:col-span-2">
              <div className="flex items-center gap-3 mb-6 cursor-pointer" onClick={() => { onNavigate('landing'); window.scrollTo(0, 0); }}>
                <img src="/assets/logo.png" alt={window.__t("المحامي")} className="w-8 h-8 rounded object-contain" />
                <span className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{window.__t("المحامي")}</span>
              </div>
              <p className={isDark ? 'text-slate-500' : 'text-slate-600'}>{window.__t("المنصة الأولى في تونس التي تستخدم الذكاء الاصطناعي التوليدي لمساعدة المحامين في تحقيق العدالة بكفاءة وسرعة.")}</p>
            </div>
            <div>
              <h4 className={`${isDark ? 'text-white' : 'text-slate-900'} font-bold mb-6`}>{window.__t("روابط سريعة")}</h4>
              <ul className={`space-y-3 ${isDark ? 'text-slate-500' : 'text-slate-600'}`}>
                <li><button onClick={() => { onNavigate('landing'); window.scrollTo(0, 0); }} className="hover:text-yellow-400 transition">{window.__t("الرئيسية")}</button></li>
                <li><button onClick={() => scrollToSection('features')} className="hover:text-yellow-400 transition">{window.__t("عن المنصة")}</button></li>
                <li><button onClick={() => scrollToSection('pricing')} className="hover:text-yellow-400 transition">{window.__t("الأسعار")}</button></li>
                <li><button onClick={() => scrollToSection('contact')} className="hover:text-yellow-400 transition">{window.__t("اتصل بنا")}</button></li>
              </ul>
            </div>
            <div>
              <h4 className={`${isDark ? 'text-white' : 'text-slate-900'} font-bold mb-6`}>{window.__t("قانوني")}</h4>
              <ul className={`space-y-3 ${isDark ? 'text-slate-500' : 'text-slate-600'}`}>
                <li><button onClick={() => onNavigate('terms')} className="hover:text-yellow-400 transition">{window.__t("شروط الاستخدام")}</button></li>
                <li><button onClick={() => onNavigate('privacy')} className="hover:text-yellow-400 transition">{window.__t("سياسة الخصوصية")}</button></li>
              </ul>
            </div>
          </div>
      ...
      {isVideoModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setIsVideoModalOpen(false)}>
          <div className={`relative w-full max-w-4xl ${isDark ? 'bg-slate-900' : 'bg-white'} rounded-2xl shadow-2xl overflow-hidden`} onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setIsVideoModalOpen(false)} className={`absolute top-4 start-4 z-10 p-2 rounded-full ${isDark ? 'bg-slate-800 hover:bg-slate-700 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-900'} transition`}>
